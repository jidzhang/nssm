...
int remove_service(nssm_service_t* service)
{
	...
	/* Open the service with stop + query + delete access */
	service->handle = open_service(services, service->name, DELETE | SERVICE_STOP | SERVICE_QUERY_STATUS, service->name, _countof(service->name));
	if (!service->handle)
	{
		return 3;
	}
	...
	if (QueryServiceStatus(service->handle, &status))
	{
		...
		{
			SERVICE_STATUS ignore;
			ControlService(service->handle, SERVICE_CONTROL_STOP, &ignore);

			/* Wait up to 30 seconds for the service to stop */
			unsigned long waited = 0;
			while (waited < 30000)
			{
				if (!QueryServiceStatus(service->handle, &status)) break;
	...
	if (!DeleteService(service->handle))
	{
		...
		if (error == ERROR_SERVICE_MARKED_FOR_DELETE)
		{
			...
			while (waited < 10000)
			{
				...
				if (GetLastError() != ERROR_SERVICE_MARKED_FOR_DELETE) break;
			}
			_ftprintf(stderr, _T("%s: %s\n"), service->name, error_string(ERROR_SERVICE_MARKED_FOR_DELETE));
			return 4;
		}
...
int start_service(nssm_service_t* service)
{
	...
	*/
	...
	if (use_critical_section) LeaveCriticalSection(&service->throttle_section);
