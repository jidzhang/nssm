...
much like srvany. But NSSM is the Native System Service Manager and can take
...
1. First NSSM will attempt to generate a Control-C event and send it to the
...
2. Secondly NSSM will enumerate all windows created by the application and send
   them a `WM_CLOSE` message, requesting a graceful exit.
3. Thirdly NSSM will enumerate all threads created by the application and send
   ...
   method.
4. Finally NSSM will call `TerminateProcess()` to request that the operating
...
but any of the other values are omitted they will receive sensible defaults.
...
- **If `<service>\Environment` is defined:** The contents of `Environment` are MERGED into the environment.
- **If `<service>\Parameters\AppEnvironment` is defined:** The service inherits the environment specified in `AppEnvironment`.
- **If `<service>\Parameters\AppEnvironmentExtra` is defined:** The contents of `AppEnvironmentExtra` are APPENDED to the environment.
...
- **Start** - Triggered when the service is requested to start.
  - **\*Pre** - Called before NSSM attempts to launch the application.
  - **Post** - Called after the application successfully starts.

- **Stop** - Triggered when the service is requested to stop.
  - **\*Pre** - Called before NSSM attempts to kill the application.

- **Exit** - Triggered when the application exits.
  - **\*Post** - Called after NSSM has cleaned up the application.

- **Rotate** - Triggered when online log rotation is requested.
  - **\*Pre** - Called before NSSM rotates logs.
  - **Post** - Called after NSSM rotates logs.

- **Power**
  - **Change** - Called when the system power status has changed.
  - **Resume** - Called when the system has resumed from standby.
...
To configure the service to stop when the application exits with an
exit code of 2, run
...
to remove a service. You will be prompted for confirmation before the service
...
edit the `nssm.vcproj` and `nssm90.sln` files to set a lower version number.
They are known not to work with default settings.

Production builds use Visual Studio 2008 (`nssm90.sln` / `nssm.vcproj`).
A Visual Studio 2019 solution (`nssm140.sln` / `nssm.vcxproj`) is also
provided for development and testing.

Four build configurations are available:
...
Output goes to `out/<Config>/win32|win64/`.
...
Note that `version.h` is auto-generated by `version.cmd` during the pre-build
step. It calls `git describe --tags --long` to determine the version.
If not on a tagged commit the version will show as prerelease.

## Testing

NSSM includes a unit test suite (catch2) and an integration test suite
(Pester 3.4, which ships inbox on Windows 10).

To build and run all tests from a single command:
...
To clean all build and test artifacts:

```cmd
clean.bat
```

To run integration tests only (from an admin PowerShell):
...
```powershell
powershell -ExecutionPolicy Bypass -File tests\integration\run_tests.ps1 -Tests "Lifecycle"
```
...
- Production code: C++03 (must compile with VS2008+).
- Test code: C++11 or later is acceptable.
- No static local variables in production code.
- Function/variable names use PascalCase (variables start lowercase).
- Macros use UPPER_SNAKE_CASE.

Submitting changes:

- Keep commits focused and atomic.
- Run `run_all_tests.bat` before submitting. All tests must pass.
- Do not modify `version.h` directly; it is auto-generated.
- Do not add dependencies beyond the Windows SDK, `psapi.lib` and
  `shlwapi.lib`.
...
|------|-------------|
| `*.cpp`, `*.h` | Source code |
| `nssm90.sln`/`vcproj` | VS2008 solution (production) |
| `nssm140.sln`/`vcxproj` | VS2019+ solution (development) |
| `version.cmd` | Auto-generates `version.h` from git tags |
| `messages.mc` | Event log message definitions (EN/FR/IT) |
...
- Since version 2.26, NSSM has been hardened against several security
